...
def letter_stat(our_str):
    """
    Это функция, которая на вход принимает строку our_str и возращает словарь letters_dict,
    где в качестве ключей буквы строки, а значениями являются число , повторения этой буквы.
    :param our_str: на вход строка
    :return: на выход словарь ключь - буква, значение- количество повторений
    """
    letters_dict = {}
    keyname = None
    for i in range(0, len(our_str)):
        keyname = our_str[i]
        letters_dict[keyname] = our_str.count(our_str[i])
    return letters_dict
