...
class PersonInfo:
    ...
    def __init__(self, familia: str, vozrast: int, *args):
        self.F_I = familia
        self.el = args
    ...
    def path_deps(self):
        if len(self.el) == 4:
            return f'{self.el[0]} --> {self.el[1]} --> {self.el[2]} --> {self.el[3]}'
        elif len(self.el) == 3:
            return f'{self.el[0]} --> {self.el[1]} --> {self.el[2]}'
        elif len(self.el) == 2:
            return f'{self.el[0]} --> {self.el[1]}'
        elif len(self.el) == 1:
            return f'{self.el[0]}'
    ...
    def new_salary(self):
        ...
        spisok = []
        slovar = {}
        for elem in range(0, len(str1)):
            slovar[str1[elem]] = str1.count(str1[elem])
        for key, value in slovar.items():
            spisok.append(value)
        spisok.sort(reverse=True)
        spisok = sum(spisok[:3])
        return 1337 * self.vozrast * spisok
